...
import { Eye, FilePenLine, LogOut, Paperclip, Plus, ShieldCheck, Trash2, Users, X } from "lucide-react";
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout }) => {
  ...
  const handleUploadNote = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    const payload = {
      title: noteForm.title || file.name.replace(/\.[^/.]+$/, ""),
      summary: noteForm.summary || `Uploaded note: ${file.name}`,
      unit: noteForm.unit || "Uploaded Notes",
      pages: Number(noteForm.pages) || 1,
      author: noteForm.author || "StudyHub Admin",
      subject: noteForm.subject || "General",
      year: noteForm.year || "SY",
      fileName: file.name,
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to upload note.");
      setMessage("Note uploaded successfully.");
      setNoteForm(emptyNote);
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to upload note.");
    } finally {
  ...
  const handleUploadPyq = async (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0];
    if (!file) return;
    const payload = {
      subject: pyqForm.subject || "General",
      collegeYear: pyqForm.collegeYear || "SY",
      yearOfExam: pyqForm.yearOfExam || new Date().getFullYear(),
      semester: Number(pyqForm.semester) || 3,
      examType: pyqForm.examType || "End-Term",
      duration: pyqForm.duration || "3 hours",
      totalMarks: Number(pyqForm.totalMarks) || 100,
      solved: Boolean(pyqForm.solved),
      fileName: file.name,
      fileContent: await readFileAsBase64(file),
    };
    ...
    try {
      const response = await fetch("/api/admin/pyqs/upload", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Unable to upload PYQ.");
      setMessage("PYQ uploaded successfully.");
      setPyqForm(emptyPyq);
      await load();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to upload PYQ.");
    } finally {
  ...
  return <div className="min-h-screen bg-slate-950 text-slate-900">
    <header className="border-b border-slate-800 bg-slate-900 px-4 py-4 text-white sm:px-8"><div className="mx-auto flex max-w-7xl items-center justify-between"><div><div className="flex items-center gap-2 text-xl font-bold"><img src="/studyhub-logo.svg" alt="StudyHub" className="h-12 w-[92px] object-contain object-left" /><span className="rounded-full bg-amber-400/15 px-2 py-1 text-[10px] uppercase tracking-wider text-amber-300">Admin</span></div><p className="mt-1 text-xs text-slate-400">Content and user management console</p></div><button onClick={onLogout} className="flex items-center gap-2 rounded-lg border border-slate-700 px-3 py-2 text-xs font-semibold text-slate-200 hover:bg-slate-800"><LogOut className="h-4 w-4" /> Logout</button></div></header>
